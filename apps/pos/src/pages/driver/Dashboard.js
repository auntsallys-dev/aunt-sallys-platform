...
export function DriverDashboardPage() {
            ...
            if (prev.length > 0) {
                for (const newOrder of newOrders) {
                    ...
                    if (newOrder.status === "out_for_delivery" &&
                        ...
                        setDeliveryPrompt((curr) => curr ?? newOrder);
                    }
                }
            }
    ...
    async function handleClaimOrder(order, e) {
        e.stopPropagation();
    ...
    return (_jsxs("div", { className: "min-h-screen bg-gray-50", children: [_jsxs("header", { className: "sticky top-0 z-10 border-b border-gray-200 bg-white px-4 py-3", children: [_jsxs("div", { className: "flex items-center justify-between", children: [_jsxs("div", { children: [_jsxs("div", { className: "text-sm font-bold text-gray-900", children: [user?.firstName, " ", user?.lastName] }), _jsx("div", { className: "text-xs text-gray-400", children: "Driver" })] }), _jsxs("div", { className: "flex items-center gap-3", children: [_jsx("button", { onClick: toggleTracking, className: `rounded-xl px-3 py-1.5 text-xs font-semibold transition-colors ${tracking
                                            ...
                                            : "bg-gray-100 text-gray-600 hover:bg-gray-200"}`, children: tracking ? "● Tracking On" : "Start Tracking" }), _jsx("button", { onClick: handleLogout, className: "text-xs text-gray-400 hover:text-gray-600", children: "Sign Out" })] })] }), gpsStatus === "error" && (_jsx("div", { className: "mt-2 rounded-lg bg-red-50 px-3 py-1.5 text-xs text-red-600", children: "GPS unavailable \u2014 check location permissions" }))] }), _jsxs("div", { className: "px-4 py-5", children: [_jsxs("div", { className: "mb-4 flex items-center justify-between", children: [_jsx("h1", { className: "text-lg font-bold text-gray-900", children: "Delivery Orders" }), _jsx("button", { onClick: () => { setLoading(true); fetchOrders(); }, className: "text-xs text-brand-600 hover:text-brand-700", children: "Refresh" })] }), error && (_jsx("div", { className: "mb-4 rounded-xl bg-red-50 px-4 py-3 text-sm text-red-700", children: error })), loading && (_jsx("div", { className: "py-12 text-center text-gray-400", children: "Loading orders\u2026" })), !loading && orders.length === 0 && (_jsxs("div", { className: "py-12 text-center", children: [_jsx("div", { className: "text-4xl mb-3", children: "\uD83D\uDE97" }), _jsx("p", { className: "text-gray-500", children: "No delivery orders assigned." })] })), _jsx("div", { className: "space-y-3", children: orders.map((order) => (_jsxs("div", { onClick: () => navigate(`/driver/orders/${order.id}`), className: "rounded-2xl bg-white p-4 shadow-sm ring-1 ring-gray-100 active:bg-gray-50 cursor-pointer transition-colors", children: [_jsxs("div", { className: "mb-2 flex items-center justify-between", children: [_jsxs("div", { className: "flex items-center gap-2", children: [_jsx("span", { className: "font-mono text-sm font-semibold text-gray-900", children: order.orderNumber }), order.isAssignedToMe && (_jsx("span", { className: "rounded-full bg-brand-100 px-2 py-0.5 text-xs font-medium text-brand-700", children: "Assigned" }))] }), _jsx("span", { className: `rounded-full px-2.5 py-0.5 text-xs font-medium ${STATUS_COLORS[order.status] ?? "bg-gray-100 text-gray-600"}`, children: order.status.replace(/_/g, " ") })] }), _jsx("div", { className: "text-sm font-medium text-gray-800", children: order.customerName }), order.customerPhone && (_jsx("a", { href: `tel:${order.customerPhone}`, onClick: (e) => e.stopPropagation(), className: "mt-1 text-xs text-brand-600 hover:underline", children: order.customerPhone })), _jsxs("div", { className: "mt-1 text-xs text-gray-400", children: [order.items?.length ?? 0, " item", order.items?.length !== 1 ? "s" : "", " · ", "\u20B1", parseFloat(order.total).toFixed(2)] }), !order.isAssignedToMe && (_jsx("button", { onClick: (e) => handleClaimOrder(order, e), disabled: claimingId === order.id, className: "mt-3 w-full rounded-xl bg-brand-600 py-2 text-xs font-semibold text-white hover:bg-brand-700 disabled:opacity-60 transition-colors", children: claimingId === order.id ? "Claiming…" : "🙋 Claim this Order" }))] }, order.id))) })] }), deliveryPrompt && (_jsx("div", { className: "fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4", children: _jsxs("div", { className: "w-full max-w-sm rounded-2xl bg-white shadow-2xl p-6 text-center", children: [_jsx("div", { className: "text-4xl mb-3", children: "\uD83D\uDE9A" }), _jsx("h2", { className: "text-lg font-bold text-gray-900 mb-2", children: "Ready for Delivery!" }), _jsxs("p", { className: "text-sm text-gray-500 mb-6", children: ["Order ", _jsx("strong", { children: deliveryPrompt.orderNumber }), " is ready for delivery. Are you now delivering?"] }), _jsxs("div", { className: "flex gap-3", children: [_jsx("button", { onClick: () => setDeliveryPrompt(null), className: "flex-1 rounded-xl border border-gray-200 py-3 text-sm text-gray-500 hover:bg-gray-50", children: "Not Yet" }), _jsx("button", { onClick: () => startDeliveryTracking(deliveryPrompt), className: "flex-1 rounded-xl bg-green-600 py-3 text-sm font-bold text-white hover:bg-green-700", children: "\u2705 Yes, Start Delivery" })] })] }) }))] }));
}
