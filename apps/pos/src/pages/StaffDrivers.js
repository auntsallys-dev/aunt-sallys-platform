...
export function StaffDriversPage() {
    ...
    // Load drivers and locations for this branch
    useEffect(() => {
        if (!selectedBranchId)
            return;
        api.admin.drivers.list(selectedBranchId)
            .then((res) => setDrivers(res.data))
            .catch(() => { });
        api.admin.drivers.locations(selectedBranchId)
            .then((res) => {
            const locs = new Map();
    ...
    useEffect(() => {
        ...
        const ws = new WebSocket(`${wsBase}/api/v1/ws/branch:${selectedBranchId}:drivers`);
        ws.onmessage = (event) => {
            try {
    ...
    return (_jsxs("div", { className: "flex h-full flex-col", children: [_jsxs("div", { className: "border-b border-gray-200 bg-white px-6 py-4", children: [_jsxs("div", { children: [_jsx("h1", { className: "text-xl font-bold text-gray-900", children: "Driver Tracking" }), _jsx("p", { className: "text-sm text-gray-500", children: "Live driver locations for this branch" })] }), error && (_jsx("div", { className: "mt-3 rounded-xl bg-yellow-50 px-4 py-2 text-sm text-yellow-700", children: error }))] }), _jsxs("div", { className: "flex flex-1 overflow-hidden", children: [_jsxs("div", { className: "w-56 flex-shrink-0 overflow-y-auto border-r border-gray-200 bg-white", children: [_jsx("div", { className: "px-4 py-3 border-b border-gray-100", children: _jsxs("p", { className: "text-xs font-semibold uppercase tracking-wide text-gray-400", children: [driverLocations.size, " active \u00B7 ", drivers.length, " total"] }) }), drivers.map((driver) => {
                                ...
                                return (_jsxs("div", { className: "flex items-center gap-3 border-b border-gray-50 px-4 py-3 hover:bg-gray-50 cursor-pointer", onClick: () => {
                                    ...
                                    }, children: [_jsxs("div", { className: `flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-full text-xs font-bold text-white ${isActive ? "bg-brand-600" : "bg-gray-300"}`, children: [driver.firstName[0], driver.lastName?.[0] ?? ""] }), _jsxs("div", { className: "flex-1 min-w-0", children: [_jsxs("div", { className: "text-sm font-medium text-gray-900 truncate", children: [driver.firstName, " ", driver.lastName] }), _jsx("div", { className: `text-xs ${isActive ? "text-green-600" : "text-gray-400"}`, children: isActive ? "● Active" : "Offline" })] })] }, driver.id));
                            }), drivers.length === 0 && (_jsx("div", { className: "px-4 py-8 text-center text-sm text-gray-400", children: "No drivers for this branch." }))] }), _jsx("div", { ref: mapRef, className: "flex-1" })] })] }));
}
//# sourceMappingURL=StaffDrivers.js.map