...
export function DriversPage() {
  ...
  // Load drivers and their latest locations when branch changes
  useEffect(() => {
    if (!selectedBranch) return;

    apiFetch(`/admin/drivers?branchId=${selectedBranch}`)
      .then((res) => setDrivers(res.data))
      .catch(() => {});

    apiFetch(`/admin/drivers/locations?branchId=${selectedBranch}`)
      .then((res) => {
  ...
  useEffect(() => {
    ...
    const wsUrl = `${WS_URL.replace(/^http/, "ws")}/api/v1/ws/branch:${selectedBranch}:drivers`;
