...
<body>
  ...
  <div class="footer">
    ...
    <div>Acknowledgement Certificate No.: ${inv.casAcNumber ? esc(inv.casAcNumber) : "[AC NO. — pending]"}</div>
    ${inv.casAcIssuedOn ? `<div>Date of Issuance: ${esc(new Date(inv.casAcIssuedOn).toLocaleDateString("en-PH"))}</div>` : ""}
    <div class="bold" style="margin-top:6px">"THIS INVOICE SHALL BE VALID FOR<br>FIVE (5) YEARS FROM THE DATE OF THE<br>ACKNOWLEDGEMENT CERTIFICATE"</div>
    <div class="small" style="margin-top:6px">
      Software Provider (Maker):<br>
      J.M. BONNEVIE LABS COMPUTER PROGRAMMING SOLUTIONS OPC<br>
      TIN: 805-847-865-00000<br>
      BIR Accreditation No.: ${esc(process.env.POS_SUPPLIER_ACCRED_NO || "[pending]")}
    </div>
